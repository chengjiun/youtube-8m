...
flags.DEFINE_integer(
    "nextvlad_hidden_size", 1024, "Number of units in the NeXtVLAD hidden layer."
)
...
class NeXtVLAD:
    ...
    def forward(self, input, mask=None):
        ...
        cluster_weights2 = tf.compat.v1.get_variable(
            "cluster_weights2",
...
class NeXtVLADModel(models.BaseModel):
    ...
    def create_model(
        ...
        **unused_params
        ...
        gating_weights_1 = tf.compat.v1.get_variable(
            "gating_weights_1",
        ...
        gating_weights_2 = tf.compat.v1.get_variable(
            "gating_weights_2",
        ...
        **unused_params
        ...
        for n in range(mix_number):
            with tf.variable_scope("branch_%d" % n):
                res = self.nextvlad_model(
                    video_ftr=model_input[:, :, 0:1024],
        ...
        mix_logit = tf.reduce_sum(
            tf.multiply(tf.expand_dims(mix_weights, -1), logits), axis=1
        )

        pred = tf.nn.sigmoid(mix_logit)

        if is_training:
            rank_pred = tf.expand_dims(
                tf.nn.softmax(tf.div(mix_logit, FLAGS.cl_temperature), axis=-1), axis=1
            )
            aux_rank_preds = tf.nn.softmax(
                tf.div(logits, FLAGS.cl_temperature), axis=-1
            )
            epsilon = 1e-8
