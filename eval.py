...
def build_graph(
    ...
    offset = np.array([4.0 / 512] * 1024 + [0] * 128)
    offset = tf.constant(offset, dtype=tf.float32)

    eigen_val = tf.constant(
        np.sqrt(np.load("yt8m_pca/eigenvals.npy")[:1024, 0]), dtype=tf.float32
    )
    model_input = tf.multiply(
        model_input_raw - offset,
        tf.pad(eigen_val + 1e-4, [[0, 128]], constant_values=1.0),
    )
...
def evaluate():
    ...
    with tf.Graph().as_default():
        ...
        model = find_class_by_name(
            flags_dict["model"], [frame_level_models, video_level_models, nextvlad]
        )()
        ...
        while True:
            last_global_step_val = evaluation_loop(
                fetches, saver, summary_writer, evl_metrics, last_global_step_val
            )
            if FLAGS.run_once:
                break
