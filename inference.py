...
if __name__ == "__main__":
    ...
    flags.DEFINE_integer(
        "segment_max_pred", 100000, "Limit total number of segment outputs per entity."
    )
    flags.DEFINE_string(
    ...
    # Other flags.
    flags.DEFINE_integer("batch_size", 512, "How many examples to process per batch.")
    flags.DEFINE_integer(
...
def get_input_data_tensors(reader, data_pattern, batch_size, num_readers=1):
  ...
  """
    with tf.name_scope("input"):
        ...
        input_data_dict = tf.train.batch_join(
            examples_and_labels,
            batch_size=batch_size,
            allow_smaller_final_batch=True,
            enqueue_many=True,
...
def get_segments(batch_video_mtx, batch_num_frames, segment_size):
    ...
    segment_mask = 0 < (padded_segment_sizes[:, np.newaxis] - np.arange(0, max_frame))
...
def inference(reader, train_dir, data_pattern, out_file_location, batch_size, top_k):
    with tf.Session(
        ...
        inference_model_name = (
            "segment_inference_model" if FLAGS.segment_labels else "inference_model"
        )
        checkpoint_file = os.path.join(
            FLAGS.train_dir, "inference_model", inference_model_name
        )
        ...
        try:
            while not coord.should_stop():
                ...
                logging.info(
                    ...
                    + " examples/sec: %.2f" % (num_examples_processed / elapsed_time)
                )
                for line in format_lines(
                    video_id_batch_val, predictions_val, top_k, whitelisted_cls_mask
                ):
                    out_file.write(line)
                out_file.flush()
            ...
            if FLAGS.segment_labels:
                ...
                logging.info(
                    "Writing sorted segment predictions to: %s", final_out_file.name
                )
